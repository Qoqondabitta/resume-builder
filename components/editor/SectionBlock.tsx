...
export default function SectionBlock({
  section, data, onDataChange, onRename, onDelete, onSectionChange, dragControls,
}: SectionBlockProps) {
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 bg-gray-50 border-b border-gray-100">
        ...
        {editingTitle ? (
          <div className="flex items-center gap-1.5 flex-1 min-w-0">
            ...
            />
            <button onClick={commitTitle} className="shrink-0 text-green-500 hover:text-green-600 transition-colors">
              <Check size={13} />
            </button>
          </div>
        ) : (
          <button
            ...
            className="flex-1 text-left text-xs font-bold text-gray-800 hover:text-primary-600 transition-colors group flex items-center gap-1.5 min-w-0"
  ...
  switch (section.type) {
    ...
    case 'experience':
      return (
        <ExperienceEditor
          ...
          onChange={(items) => onDataChange((d) => ({ ...d, experience: items }))}
        />
      );
    ...
    case 'education':
      return (
        <EducationEditor
          ...
          onChange={(items) => onDataChange((d) => ({ ...d, education: items }))}
        />
      );
    ...
    case 'skills':
      return (
        <SkillsEditor
          skills={data.skills}
          onChange={(skills) => onDataChange((d) => ({ ...d, skills }))}
        />
      );
    ...
    case 'projects':
      return (
        <ProjectsEditor
          items={data.projects}
          onChange={(items) => onDataChange((d) => ({ ...d, projects: items }))}
        />
      );
    ...
    case 'achievements':
      return (
        <ListEditor
          ...
          onChange={(items) => onDataChange((d) => ({ ...d, achievements: items }))}
    ...
    case 'languages':
      return (
        <ListEditor
          ...
          onChange={(items) => onDataChange((d) => ({ ...d, languages: items }))}
    ...
    case 'certifications':
      return (
        <ListEditor
          ...
          onChange={(items) => onDataChange((d) => ({ ...d, certifications: items }))}
    ...
    case 'custom':
      return (
        <ListEditor
          items={section.customContent ?? []}
          onChange={(items) => onSectionChange({ ...section, customContent: items })}
...
function ExperienceEditor({
  ...
  const editBullet = (i: number, bi: number, val: string) =>
    update(i, { bullets: items[i].bullets.map((b, j) => (j === bi ? val : b)) });
  const dropBullet = (i: number, bi: number) =>
    update(i, { bullets: items[i].bullets.filter((_, j) => j !== bi) });
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {(
              [
                ...
                { k: 'location', ph: 'Location'                      },
                { k: 'period',   ph: 'Period  e.g. Jan 2022–Present' },
              ] as const
            ).map(({ k, ph }) => (
              <FieldInput
                key={k}
            ...
            <div className="space-y-1.5">
              {exp.bullets.map((b, bi) => (
                <div key={bi} className="flex items-center gap-1.5">
                  <span className="text-gray-300 text-xs shrink-0 select-none">•</span>
                  <input
                    value={b}
                    onChange={(e) => editBullet(i, bi, e.target.value)}
                    placeholder="Describe your impact…"
                    className={inputCls}
                  />
                  <button onClick={() => dropBullet(i, bi)} className={iconBtn('red')}>
                    <X size={11} />
                  </button>
                </div>
              ))}
...
function EducationEditor({
  items, onChange,
}: { items: Education[]; onChange: (items: Education[]) => void }) {
  const blank = (): Education => ({
    degree: '', school: '', location: '', period: '', note: '',
  });
  ...
  return (
    <div className="space-y-3">
      {items.map((edu, i) => (
        <div key={i} className="border border-gray-100 rounded-xl p-3 space-y-2 bg-gray-50/40 hover:border-gray-200 transition-colors">
          <EntryHeader label={`Entry ${i + 1}`} onRemove={() => onChange(items.filter((_, j) => j !== i))} />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {(
              [
                { k: 'degree',   ph: 'Degree / Certification'   },
                { k: 'school',   ph: 'School / Institution'     },
                { k: 'location', ph: 'Location'                 },
                { k: 'period',   ph: 'Period  e.g. 2014–2018'   },
              ] as const
            ).map(({ k, ph }) => (
              <FieldInput
                key={k}
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-1.5 min-h-[32px]">
        ...
        {skills.length === 0 && (
          <p className="text-xs text-gray-300 italic">No skills yet — add some below</p>
        )}
      </div>
      <div className="flex gap-2">
        ...
        <button
          onClick={add}
          className="px-3 py-1.5 text-xs font-semibold text-white bg-primary-600 rounded-lg hover:bg-primary-700 active:scale-95 transition-all"
        >
          Add
        </button>
      </div>
...
function ProjectsEditor({
  items, onChange,
}: { items: Project[]; onChange: (items: Project[]) => void }) {
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1 mb-1.5">
              {proj.tech.map((t) => (
                <button
                  key={t}
                  onClick={() => update(i, { tech: proj.tech.filter((x) => x !== t) })}
                  title="Click to remove"
...
function EntryHeader({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <button onClick={onRemove} className={iconBtn('red')} title="Remove">
        <X size={13} />
      </button>
    </div>
  );
...
function LogoUpload({
  label, url, onUrl,
}: { label: string; url?: string; onUrl: (url: string) => void }) {
  return (
...
function AddRowBtn({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-1.5 text-[11px] text-primary-500 hover:text-primary-700 font-semibold transition-colors mt-1.5"
    >
      <Plus size={11} /> {label}
    </button>
  );
...
function AddSectionBtn({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border-2 border-dashed border-gray-200 text-xs text-gray-400 font-semibold hover:border-primary-300 hover:text-primary-500 hover:bg-primary-50/50 active:scale-[0.98] transition-all"
    >
      <Plus size={13} /> {label}
    </button>
  );
...
const inputCls =
  'text-xs text-gray-700 border border-gray-200 rounded-lg px-2.5 py-1.5 focus:border-primary-300 focus:ring-1 focus:ring-primary-100 outline-none bg-white transition-all';

const iconBtn = (color: 'red' | 'gray') =>
  color === 'red'
    ? 'p-0.5 text-gray-300 hover:text-red-400 transition-colors shrink-0'
    : 'p-0.5 text-gray-300 hover:text-gray-600 transition-colors shrink-0';
